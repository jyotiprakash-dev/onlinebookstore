...
import com.bittercode.util.UserInputValidator;
import com.bittercode.util.UserInputValidator.ValidationResult;
...
public class CustomerRegisterServlet extends HttpServlet {
    ...
    public void service(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
        ...
        ValidationResult validation = UserInputValidator.validateCustomerRegistration(mailId, pWord, fName, lName, addr,
                phNo, acceptance);
        if (!validation.isValid()) {
            RequestDispatcher rd = req.getRequestDispatcher("CustomerRegister.html");
            rd.include(req, res);
            pw.println("<table class=\"tab\"><tr><td>" + validation.getMessage() + "</td></tr></table>");
            return;
        }

        mailId = UserInputValidator.trimToEmpty(mailId);
        fName = UserInputValidator.trimToEmpty(fName);
        lName = UserInputValidator.trimToEmpty(lName);
        addr = UserInputValidator.trimToEmpty(addr);
        phNo = UserInputValidator.trimToEmpty(phNo);
